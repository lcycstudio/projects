...
def chapter_image_path(instance, filename):
    ...
    return 'physics11/chapter/{0}/{1}'.format(instance.chapter, new_filename)


def section_image_path(instance, filename):
    section_path = '{0}/section/{1}/{2}'.format(
        settings.PHYS11_ROOT, instance.chapter, instance.section)
    ...
    return 'physics11/section/{0}/{1}/{2}'.format(instance.chapter, instance.section, filename)
