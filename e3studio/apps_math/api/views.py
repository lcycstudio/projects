...
from apps_math.models import AppsMath, Function, Parameter, InitialFunctionValue, InitialDerivativeValue, LeftBound, RightBound, Title, HorizontalLabel, VerticalLabel
from .serializers import AppsMathSerializer, OneAppMathSerializer, CFSerializer, ODESerializer
...
class AppsMathListView(ListAPIView):
    queryset = AppsMath.objects.all()
    serializer_class = AppsMathSerializer
    permission_classes = (permissions.AllowAny, )

    def get(self, request, format=None):
        appsmathset = [item for item in AppsMath.objects.all()]
        serializer = AppsMathSerializer(appsmathset, many=True)
        serializer_data = serializer.data
        return Response(serializer_data)


class AppsMathDetailView(RetrieveAPIView):
    queryset = AppsMath.objects.all()
