...
class UserLoginSerializer(serializers.Serializer):
    ...
    def _validate_email(self, email, password):
        ...
        if email and password:
            try:
                get_user = User.objects.get(email__icontains=email)
        ...
        # if 'company' not in email.split("@")[1]:
        #     msg = _('Please enter a company email.')
        #     raise exceptions.ValidationError({'error': msg})
        # else:
        return user
...
class UserRegisterSerializer(serializers.Serializer):
    ...
    def save(self, request):
        ...
        student, created = Profile.objects.get_or_create(
            user=user,
            defaults={
