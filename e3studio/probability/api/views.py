...
from probability.models import Chapter, Section, Paragraph, Image, Exercise, Choice, Answer
from .serializers import ChapterSerializer, SectionSerializer, ChoiceSerializer, AnswerSerializer, CodingSerializer
...
class ContentDetailView(RetrieveAPIView):
    def get(self, request, chapter, section, format=None):
        ...
        for index in range(len(sectionset)):
            if section.id == sectionset[index].id:
                if section.id != 1 and index != 0:
        ...
        paragraphset = Paragraph.objects.filter(
            chapter=chapter_obj, section=section)
        imageset = Image.objects.filter(
            chapter=chapter_obj, section=section)
        for item in paragraphset:
            serializer_data["paragraphs"].append(item.paragraph)
    ...
    def put(self, request, chapter, section, pk, format=None):
        ...
        if "plt." not in data_list and "=" not in data_list[index]:
            try:
                ...
                if "import" not in data_list[index]:
                    exec("temp_dict['value'] =" + data_list[index])
                    if "print" in data_list[index]:
                        temp_dict['value'] = "Print function is disabled. Type '{0}' in the next line.".format(
                            data_list[index][data_list[index].index("(") + 1:len(data_list[index]) - 1])
                    if type(temp_dict['value']) == type:
                        temp_dict['value'] = "<class '{0}'>".format(
                            temp_dict['value'].__name__)
                else:
                    temp_dict['value'] = "Please give more commands."
                obj, created = CodeText.objects.update_or_create(
                    chapter=chapter_obj, section=section_obj,
            ...
            return Response(temp_dict)
        if "plt." not in data_list and "=" in data_list[index]:
            try:
                exec(request.data["code"])
                if "print" in data_list[index]:
                    temp_dict['value'] = "Print function is disabled."
                else:
                    index_of_equal = data_list[index].index("=")
                    temp_dict["value"] = "Please type {x} in a new line to show its value.".format(
                        x=data_list[index][:index_of_equal])
                obj, created = CodeText.objects.update_or_create(
                    chapter=chapter_obj, section=section_obj,
                    id=pk, defaults={'codetext': request.data["code"], 'plot': "", 'value': temp_dict["value"]})
            except Exception as inst:
